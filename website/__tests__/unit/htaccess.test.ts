...
describe(".htaccess security headers", () => {
  ...
  it("routes jaatumeinaaya admin routes to PHP backend and redirects old admin", () => {
    expect(htaccess).toMatch(/RewriteRule \^jaatumeinaaya\/\?\$ backend\/admin\/index\.php/);
    expect(htaccess).toMatch(/RewriteRule \^jaatumeinaaya\/login/);
    expect(htaccess).toMatch(/RewriteRule \^admin\(\/panel\)\?\/\?\$ \/jaatumeinaaya\//);
  });

  it("enforces clean extensionless URLs by redirecting direct .php requests", () => {
    expect(htaccess).toMatch(/RewriteCond %\{THE_REQUEST\} \\s\/jaatumeinaaya\/.*\\\.php/);
    expect(htaccess).toMatch(/RewriteRule \^jaatumeinaaya\/\(\[a-zA-Z0-9_-\]\+\)\\\.php\$ \/jaatumeinaaya\/\$1 \[R=301,L\]/);
  });

  it("blocks direct external access to internal backend directory and sensitive files", () => {
    expect(htaccess).toMatch(/RewriteCond %\{THE_REQUEST\} \\s\/backend\//);
    expect(htaccess).toMatch(/RewriteRule \^backend\(\/\.\*\)\?\$ - \[F,L\]/);
    expect(htaccess).toMatch(/FilesMatch.*sql\|env\|ini\|log\|sh\|py\|md\|git/);
  });
});
