    ...
    <>
      ...
      <section className="bg-gradient-to-b from-primary-soft via-white to-white">
        <div className="mx-auto max-w-6xl px-4 pb-16 pt-16 sm:px-6 md:pt-24">
          ...
          <h1 className="mt-4 max-w-4xl font-heading text-3xl font-bold leading-tight tracking-tight text-ink sm:text-5xl lg:text-6xl break-words">
            {config.h1}
          </h1>
          ...
          <div className="mt-8 flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 sm:gap-4">
            <Link
              href="/contact/"
              className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-6 py-3.5 font-semibold text-white shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-light text-center"
            >
              <span>Get Free Ahmedabad Consultation</span>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
          {config.capabilities.map((item, i) => (
            <Reveal key={item.title} delay={i * 80}>
              <div className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      {config.table && (
        <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
            <table className="w-full min-w-[540px] sm:min-w-0 text-left text-sm">
              <thead className="bg-slate-100 border-b border-slate-200">
                <tr>
                  {config.table.headers.map((h, idx) => (
                    <th key={h} className={`p-3.5 sm:p-4 font-semibold ${idx === 1 ? "text-primary" : "text-ink"}`}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {config.table.rows.map((row, idx) => (
                  <tr key={idx}>
                    <td className="p-3.5 sm:p-4 font-medium text-slate-800">{row.col1}</td>
                    <td className="p-3.5 sm:p-4 font-semibold text-ink">{row.col2}</td>
                    <td className="p-3.5 sm:p-4 text-slate-600">{row.col3}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <div className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary-soft/80 to-white p-5 sm:p-8 md:p-12">
          ...
          <div className="mt-6 flex flex-wrap gap-2 sm:gap-2.5">
            {ahmedabadLocalities.map((loc) => (
              <span key={loc} className="rounded-full bg-white border border-primary/20 px-3 sm:px-3.5 py-1.5 text-[11px] sm:text-xs font-semibold text-slate-800 shadow-sm">
                📍 {loc}
              </span>
            ))}
          </div>
