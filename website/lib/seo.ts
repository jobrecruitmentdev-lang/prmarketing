...
/* WebPage Schema (About, Contact, Portfolio, Tools, Directory) */
export function webPageSchema(type: "AboutPage" | "ContactPage" | "CollectionPage" | "WebPage" | "ItemPage", name: string, description: string, path: string) {
  return {
    "@context": "https://schema.org",
    "@type": type,
    name,
...
/* Master 15-Point Connected JSON-LD Schema Graph (@graph) */
export function generateConnectedGraphSchema(options: {
  pagePath: string;
  pageTitle: string;
  ...
  const pageUrl = `${site.url}${options.pagePath}`;
  ...
  const graph: any[] = [
    ...
    // 3. WebSite with Sitelinks SearchAction
    {
      "@type": "WebSite",
      "@id": websiteId,
      url: site.url,
  ...
  if (options.breadcrumbs && options.breadcrumbs.length > 0) {
    options.breadcrumbs.forEach((bc, idx) => {
      breadcrumbElements.push({
        "@type": "ListItem",
        position: idx + 2,
        name: bc.name,
        item: `${site.url}${bc.path}`,
      });
    });
    ...
    breadcrumbElements.push({
      "@type": "ListItem",
      position: 2,
      name: options.pageTitle.split("—")[0].split("|")[0].trim(),
      item: pageUrl,
    });
  }
  ...
  // 6. Service Schema (if service page)
  if (options.service) {
    graph.push({
      "@type": "Service",
      "@id": `${pageUrl}#service`,
      ...
      serviceType: options.service.serviceType || options.service.name,
      provider: {
        "@id": orgId,
      },
      areaServed: {
        "@type": "AdministrativeArea",
        name: "Ahmedabad, Gujarat, India, Worldwide",
      },
    });
  }

  // 7. FAQPage Schema (if faqs provided)
