...
description: Automatically pushes code changes and design decisions to a long-term Memory MCP graph database.
tools: [run_command, mcp]
---

## Context & Scope
This skill runs after a successful build or code change to persist architectural rules and conventions in the MCP memory layer, ensuring the agent doesn't ask the same design questions twice.

## Automated Memory Trigger Sync Constraints
- **Trigger Phase**: Immediately following a successful exit code (`exit 0`) from `./scripts/linter.sh` or `./scripts/run-tests.sh`.
- **Action**: Evaluate what files changed using `git diff --name-only`.
- **Categorization Rule**:
  - If `schema.prisma` or Pydantic models changed ➔ Invoke `mcp__create_nodes` using type `ArchitectureRule`.
  - If endpoints or hooks changed ➔ Invoke `mcp__create_nodes` using type `Convention`.
- **De-duplication Check**: If an Architecture node with a matching name property already exists, overwrite it using an update call rather than creating redundant entries.
