...
* Also, they MUST NOT clash with the reserved query parameter names.
...
**Example:** A well-formed URI Template Variable:
...
**Example:** A well-formed resource representation:
```json
{
  "_links": {
    "self": {
...
**Example:** A well-formed resource representation with custom relation fulfillment-provider:
```json
{
  "_links": {
...
## HTTP Headers
* Every HTTP Header should use Hyphenated-Pascal-Case. 
...
### API Name
* Every API Description API name MUST start with API domain enclosed in square brackets (e.g., `[API Domain] My API`). 
...
### Resource Name
* Every resource MUST have a name (defined by `x-summary` field). 
* Resource name MUST be in Title Case. 
...
### Action Name
* Every action (operation) MUST have a name (defined by `summary` field). 
* Action name MUST be in Title Case. 
* Words MUST be separated by a space.
