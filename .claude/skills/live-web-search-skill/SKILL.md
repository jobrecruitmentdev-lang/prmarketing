...
description: Fetches real-time information, runtime error documentation, and updated dependency syntax from the web.
tools: [run_command]
---
## Context & Scope
Use this skill when encountering unknown execution errors, verifying the latest versions of libraries (FastAPI, Next.js, Prisma), or looking up newly released module definitions.
## Core Query Rules
- **Intent Optimization**: Strip conversational fluff. Search using precise language errors or package keywords (e.g., "Prisma migration error P2025" or "Next.js 15 app router caching docs").
- **Prefer Source Grounding**: Prioritize official documentation domains listed in `./references/trusted-domains.md` over generic blog posts.
## Step-by-Step Process Flow
1. **Identify Gap**: Detect missing information or a failing local script log.
2. **Execute Live Search**: Trigger the search tool command using `./scripts/test-search.sh "<query>"`.
3. **Parse Snippets**: Read the returned JSON payload payload to extract code patterns.
4. **Context Injection**: Use the retrieved live data directly within the coding workspace to fix or draft target configurations.
