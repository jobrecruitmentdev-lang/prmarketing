...
description: Performs rigorous code reviews, detects security flaws, checks code quality standards, and refactors duplicated code.
tools: [read_file, write_file, run_command]
---
## Context & Scope
Runs automatically before final integration. Focuses on security, styling, documentation, and the DRY (Don't Repeat Yourself) principle.

## Step-by-Step Process
1. **Analyze target code**: Locate new or modified files.
2. **Review against Standards**: Crosscheck against `./references/style-guide.md`, `./references/review-checklist.md`, and `./references/engineering-standards.md`.
3. **Execute Static Scans**:
   - Run `./scripts/code-review.sh` to find hardcoded secrets and syntax violations.
   - Run `./scripts/find-duplicates.sh` or `./scripts/check-duplication.sh` to identify identical logic blocks.
4. **Refactor & Format**: Extract repeated code into shared modules, clean up variable names, and format syntax.
5. **Generate Report**: Output a bulleted list grouping security flaws, performance warnings, and duplication errors.
